...
> **핵심:** F1 평균 방식은 클래스별 성능을 합치는 기준이고, Log Loss는 정답 클래스에 부여한 확률의 품질을 평가한다.
...
| 개념 | 의미 |
| --- | --- |
| `micro F1` | 모든 클래스의 TP·FP·FN을 합산한 뒤 F1 Score를 계산 |
| `macro F1` | 클래스별 F1 Score를 같은 비중으로 평균 |
| `weighted F1` | 클래스별 샘플 수를 가중치로 사용해 F1 Score를 평균 |
| `Log Loss` | 정답 클래스에 부여한 예측 확률의 품질을 평가 |
...
- [ ] micro·macro·weighted F1의 계산 관점 구분하기
- [ ] 클래스 불균형에서 macro F1이 갖는 의미 설명하기
- [ ] Log Loss가 모델의 확신 정도를 반영하는 이유 설명하기
