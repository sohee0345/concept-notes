...
> **핵심:** Voting은 정해진 규칙으로 예측을 결합하고, Stacking은 메타 모델이 결합 방법을 학습한다.
...
| 개념 | 의미 |
| --- | --- |
| `Hard Voting` | 각 모델이 예측한 클래스의 다수결로 최종 클래스를 결정 |
| `Soft Voting` | 각 모델의 클래스 확률을 평균해 최종 클래스를 결정 |
| `Stacking` | 기본 모델의 예측을 입력으로 사용해 메타 모델을 학습 |
| `out-of-fold 예측` | 각 샘플을 학습에 사용하지 않은 폴드의 모델로 만든 예측 |
...
- [ ] Hard Voting과 Soft Voting의 결합 방식 비교하기
- [ ] Stacking에서 메타 모델이 사용하는 입력 설명하기
- [ ] out-of-fold 예측으로 데이터 누수를 방지하는 이유 설명하기
