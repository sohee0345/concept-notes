...
> **핵심:** 세 라이브러리는 Gradient Boosting을 기반으로 하지만 트리 확장 방식과 범주형 특성 처리 전략이 다르다.
...
| 개념 | 의미 |
| --- | --- |
| `XGBoost` | 규제와 결측값 처리 등을 강화한 Gradient Boosting 라이브러리 |
| `LightGBM` | 손실 감소가 큰 리프를 우선 확장하는 방식에 강점이 있는 라이브러리 |
| `CatBoost` | 범주형 특성 처리와 순서 기반 통계에 강점이 있는 라이브러리 |
...
- [ ] XGBoost·LightGBM·CatBoost의 대표 특징 비교하기
- [ ] LightGBM의 leaf-wise 방식에서 과적합에 주의해야 하는 이유 설명하기
- [ ] 같은 교차검증 절차로 모델을 비교해야 하는 이유 설명하기
